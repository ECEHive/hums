...
function calculateGaps(config: TemplateConfig): {
	...
	const horizontalGap =
		config.columns > 1
			? (contentWidth - totalLabelsWidth) / (config.columns - 1)
			: 0;
	const verticalGap =
		config.rows > 1
			? (contentHeight - totalLabelsHeight) / (config.rows - 1)
			: 0;
...
export function generateBarcodeLabelHTML(
	...
	const renderLabel = (
		item: (typeof itemsWithBarcodes)[0] | null,
		row: number,
		col: number,
	): string => {
		...
		return (
			...
			"in;height:" +
			config.labelHeight +
			'in;"><span class="name" style="font-size:' +
			fontSize +
			'pt">' +
			escapeHtml(item.name) +
			'</span><img class="barcode" src="' +
			item.barcodeDataURL +
			'"><span class="sku" style="font-size:' +
			skuFontSize +
			'pt">' +
			escapeHtml(item.sku || "") +
			"</span></div>"
	...
	const css =
		"@page{size:letter portrait;margin:0}*{margin:0;padding:0;box-sizing:border-box}body{font-family:Arial,sans-serif}.page{width:" +
		PAGE_WIDTH +
		"in;height:" +
		PAGE_HEIGHT +
		"in;position:relative;page-break-after:always}.page:last-child{page-break-after:auto}.label{position:absolute;display:flex;flex-direction:column;align-items:center;justify-content:center;overflow:hidden;padding:1px}.name{font-weight:bold;text-align:center;width:100%;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;line-height:1}.barcode{max-width:95%;max-height:55%;object-fit:contain}.sku{text-align:center;width:100%;line-height:1}@media print{body{-webkit-print-color-adjust:exact;print-color-adjust:exact}}";
