 ...
 */
export async function generateShiftScheduleShiftOccurrences(tx: Transaction, shiftScheduleId: number) {
    ...
    if (occurrenceIdsToDelete.length > 0) {
        await tx.delete(shiftOccurrences).where(
            inArray(shiftOccurrences.id, occurrenceIdsToDelete)
        );
    }
