import { prisma } from "@ecehive/prisma";

import z from "zod";
import type { TPermissionProtectedProcedureContext } from "../../trpc";

export const ZGetSchema = z.object({
	id: z.number().min(1),
});

export type TGetSchema = z.infer<typeof ZGetSchema>;

export type TGetOptions = {
	ctx?: TPermissionProtectedProcedureContext;
	input: TGetSchema;
};

export async function getHandler(options: TGetOptions) {
	const { id } = options.input;

	const shiftOccurrence = await prisma.shiftOccurrence.findUnique({
		where: { id },
		include: {
			attendances: {
				include: {
					user: true,
				},
			},
		},
	});

	if (!shiftOccurrence) {
		return { shiftOccurrence: undefined, assignedUsers: [] };
	}

	const assignedUsers = shiftOccurrence.attendances.map((a) => ({
		...a.user,
		assignmentStatus: a.status,
	}));

	const { attendances: _, ...occurrence } = shiftOccurrence;

	return {
		shiftOccurrence: occurrence,
		assignedUsers,
	};
}
