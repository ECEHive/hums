...
export async function createProviderHandler({ input }: { input: z.infer<typeof ZCreateProviderSchema> }) {
  return createControlProvider(input);
}

export async function deleteProviderHandler({ input }: { input: z.infer<typeof ZDeleteProviderSchema> }) {
  return deleteControlProvider(input.id);
}

export async function getProviderHandler({ input }: { input: z.infer<typeof ZGetProviderSchema> }) {
  return getControlProviderById(input.id);
}

export async function listProvidersHandler({ input }: { input: z.infer<typeof ZListProvidersSchema> }) {
  return listControlProviders(input);
}

export async function updateProviderHandler({ input }: { input: z.infer<typeof ZUpdateProviderSchema> }) {
