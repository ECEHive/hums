...
const logger = getLogger("plugin:attendance:update-shift-attendance");
...
const cronJob = new CronJob("*/1 * * * *", updateShiftAttendance);

/**
 * WorkerDefinition for the attendance plugin.
 *
 * Runs every minute to create and update shift attendance records based on
 * active user sessions and shift occurrence schedules.
 */
export const updateShiftAttendanceWorker: WorkerDefinition = {
	name: "attendance:update-shift-attendance",
	start() {
		cronJob.start();
	},
	stop() {
