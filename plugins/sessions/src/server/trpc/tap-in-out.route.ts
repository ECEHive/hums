...
import type { TKioskProtectedProcedureContext } from "@ecehive/trpc/server";
import z from "zod";
...
export async function tapInOutHandler(options: TTapInOutOptions) {
	...
	const result = await prisma.$transaction(async (tx) => {
		...
		if (tapAction === "switch_to_staffing") {
			...
			return {
				...
				newSession,
				now,
				previousType: mostRecentSession.sessionType as "regular" | "staffing",
			};
		}
	...
	// Emit events after the transaction commits (only for state-changing outcomes)
	if (result.status === "tapped_in") {
		...
		const newType =
			result.status === "switched_to_staffing" ? "staffing" : "regular";
		options.events?.emit("session:ended", {
	...
	if (
		...
		const { previousType: _pt, now: _n, ...rest } = result;
		return rest;
	}

	if (result.status === "tapped_in" || result.status === "tapped_out") {
		const { now: _n, ...rest } = result;
		return rest;
	}
